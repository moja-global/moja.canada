	...
	namespace modules {
		namespace cbm {
			...
			void PeatlandTurnoverModule::doLivePoolTurnover() {
				...
				double sphagnumMossLiveTurnover = (shrubAge - 1) <= growthParas->Rsp() ? 0.0 : growthParas->GCsp() * growthParas->NPPsp();
				double featherMossLiveTurnover = (shrubAge - 1) <= growthParas->Rfm() ? 0.0 : growthParas->GCfm() * growthParas->NPPfm();
