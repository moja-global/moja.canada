...
namespace cbm {
    ...
    std::vector<std::optional<std::string>> FlatFluxRecord::asVector() const {
        ...
        for (const auto& value : _classifierValues) {
            row.push_back(value.isNull() ? std::make_optional<std::string>() : value.value());
        }
        ...
        row.push_back(_disturbanceType.isNull() ? std::make_optional<std::string>() : pqxx::to_string(_disturbanceType.value()));
        row.push_back(_disturbanceCode.isNull() ? std::make_optional<std::string>() : pqxx::to_string(_disturbanceCode.value()));
