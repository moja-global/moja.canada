...
namespace CBM {
	...
	std::shared_ptr<ComponentBiomassCarbonCurve> VolumeToBiomassConverter::generateComponentBiomassCarbonCurve(std::shared_ptr<StandGrowthCurve> standGrowthCurve, SpeciesType speciesType){

		int standMaxAge = standGrowthCurve->standMaxAge();
		auto pf = standGrowthCurve->getPERDFactor(speciesType);

		std::shared_ptr<ComponentBiomassCarbonCurve> compomentCarbonCurve = std::make_shared<ComponentBiomassCarbonCurve>(standMaxAge);
...
/*			if (nonMerchFactor < pf->cap_nonmerch()){
				standGrowthCurve->setOkToSmooth(true);
			}*/		
			...
			double saplingFactor = Helper::calculateSaplingFactor(bioMerchStemwood + bioNonmerchStemwood, pf->k_sap(), pf->a_sap(), pf->b_sap());
			if (saplingFactor < 1)
			{
				saplingFactor = 1.0;
			}
			...
			//following handles growth curve at very young stage, and there may be no merchantable volume.
			if (age > 1 && age < minAgeForMaximumAnnualTotalMerchVol) {
				if (!nonMerchCapped){
					...
					if (preNonMerchFactor < pf->cap_nonmerch()){
						...
						else if (totalMerchVol > pf->min_volume()){							
							break;
						}
					}
				}
	...
	void VolumeToBiomassConverter::DoSmoothing(const StandGrowthCurve& standGrowthCurve, ComponentBiomassCarbonCurve* carbonCurve, SpeciesType sepciesType){		
		if (_smootherEnabled){
	...
	*/
	std::shared_ptr<ComponentBiomassCarbonCurve> VolumeToBiomassConverter::generateComponentBiomassCarbonCurve(std::shared_ptr<StandGrowthCurve> standGrowthCurve, SpeciesType speciesType, int defaultSPUID){
		// 1) query for the PERD factors
		// 2) set the PERD factors for the stand growth curve
		// 3) call generateComponentBiomassCarbonCurve(standGrowthCurve, speciesType)
		return nullptr;
	}
