...
#define _LMDIF
			...
			void LmMin::lm_initialize_control(lm_control_type * control)
			{
			...
			void LmMin::lm_minimize(int m_dat, int n_par, double *par,
				...
				void *data, lm_control_type * control)
			{
				...
				std::vector<double> fvec(m_dat), diag(n_par), fjac(n_par*m_dat), qtf(m_dat), wa1(n_par), wa2(n_par), wa3(n_par), wa4(m_dat);
				...
				lm_lmdif(m_dat, n_par, par, fvec.data(), control->ftol, control->xtol, control->gtol,
					control->maxcall * (n_par + 1), control->epsilon, diag.data(), 1,
					control->stepbound, &(control->info),
					&(control->nfev), fjac.data(), ipvt.data(), qtf.data(), wa1.data(), wa2.data(), wa3.data(), wa4.data(),
					evaluate, printout, data);

				(*printout) (n_par, par, m_dat, fvec.data(), data, -1, 0, control->nfev);
				control->fnorm = lm_enorm(m_dat, fvec.data());
				if (control->info < 0)
					control->info = 10;
...
#define SQR(x)   (x)*(x)
			...
			*
			*/
			void LmMin::lm_lmdif(int m, int n, double *x, double *fvec, double ftol,
			...
			void LmMin::lm_lmpar(int n, double *r, int ldr, int *ipvt, double *diag,
			...
			void LmMin::lm_qrfac(int m, int n, double *a, int pivot, int *ipvt,
			...
			void LmMin::lm_qrsolv(int n, double *r, int ldr, int *ipvt, double *diag,
				...
				for (j = 0; j < n; j++) {
					...
					for (k = j; k < n; k++) {
						...
						if (fabs(r[kk]) < fabs(sdiag[k])) {
							_cot = r[kk] / sdiag[k];
							_sin = p5 / std::sqrt(p25 + p25 * SQR(_cot));
							_cos = _sin * _cot;
						}
						else {
							_tan = sdiag[k] / r[kk];
							_cos = p5 / std::sqrt(p25 + p25 * SQR(_tan));
							_sin = _cos * _tan;
						}
			...
			double LmMin::lm_enorm(int n, double *x)
			{
